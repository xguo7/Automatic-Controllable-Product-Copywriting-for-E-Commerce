...
#read data
path='/home/xiaojie.guo/daren_data/1342/'
with open(path+'daren1342_clean.csv', 'r', encoding='utf-8') as f:
    orig_data=f.readlines()

workbook1=openpyxl.load_workbook(path+'assigned_topic_1342_part1_13class.xlsx')
workbook2=openpyxl.load_workbook(path+'assigned_topic_1342_part2_13class.xlsx')
...
if len(class_l)==len_: #missed the last item
    ...
    for i in range(0,len_):
        ...
        if  check_multi(tokens) <2 and len(tokens)>3 and len(tokens)<30 :
            ...
            if c_k >= 0:
                ...
                if orig_class in [5,12]:
                        new_class_l.append(0)

                if orig_class in [3,7]:
                        new_class_l.append(1) 

                if orig_class in [1,6]:
                        new_class_l.append(2)  

                if orig_class in [9]:
                        new_class_l.append(3)                                                      

                if orig_class in [0,2]:
                        new_class_l.append(4)             

                if orig_class in [4,8,10]:
    ...
    for i in range(len(new_origin)):   
        ...
        if score>0.6:
            with open(path+'assigned_write_1342/assigned_write'+str(new_class)+'.txt', 'a', encoding='utf-8') as f:
